...
# Function which loads the words allowed from the aqawords.txt file.
def LoadAllowedWords():
  # Create the AllowedWords list
  AllowedWords = []
  try:
...
def GetChoice(AllowedWords,PlayerTiles):
  Choice = ""
  valid_words = []
  time.sleep(0.5)
  print()
  print("     enter the word you would like to play")
  for word in AllowedWords:
    if CheckWordIsInTiles(word, PlayerTiles) == True: #checks if every word is in the tiles
      valid_words.append(word)
  valid_words = sorted(valid_words, key=len) #always sorts from smallest to largest
  Choice = valid_words[-1] #selects the largest word
  print("\n",Choice)
  print()
  Choice = Choice.upper()
  return Choice
...
def GetNewTileChoice():
  NewTileChoice = random.randint(1,4)    
...
def HaveTurn(PlayerName, PlayerTiles, PlayerTilesPlayed, PlayerScore, TileDictionary, TileQueue, AllowedWords, MaxHandSize, NoOfEndOfTurnTiles):
  ...
  while not ValidChoice:
    Choice = GetChoice(AllowedWords,PlayerTiles)
    if Choice == "1":
